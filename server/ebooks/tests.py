...
class ViewsTest(TestCase):
    ...
    def test_upload_view_200(self):
        ...
        with codecs.open(epub_contents_path, 'r', encoding='utf-8', errors='ignore') as f:
            print(f.name)
            # epub_file = SimpleUploadedFile("test_book.epub", bytes(f.read(), 'UTF-8'), content_type="application/epub+zip")
            data = {'epub': f}
            request = self.factory.post('upload/', data=data, format='multipart')
