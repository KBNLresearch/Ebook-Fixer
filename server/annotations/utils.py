...
def check_request_body(request):
    """Extracts the content of a request

    Args:
        request (Resquest Object): The request being sent

    Returns:
        : (Image, dict): the image object and the data being sent
    """
    try:
...
def google_vision_labels(image_path):
    ...
    Args:
        image_path (String): The path in storage to the image file
...
def azure_api_call(image_path):
    ...
    Returns:
        str, dict: The generated description and the top 10 generated labels from Azures's API with (description, score) as (key, value) # noqa: E501
    """
    analysis = None
