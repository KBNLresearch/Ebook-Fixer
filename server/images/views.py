...
def image_classification_view(request):
    ...
    Args:
        request (request object): The request object
            - uuid (String): id of e-book which the image belongs to (body)
            - filename (String): full path of image (body)
            - location (String): file name in which image appears, often html (body)
            - classification (String): classification to store for image (body)
            - raw_context (String): textual context around image (body)
