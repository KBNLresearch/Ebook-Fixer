...
import { epubCheckReportContext } from '../../helpers/EbookContext'
...
function FileUpload({ setEbookFile, setEbookId, setEbookTitle }) {
    ...
    const [epubCheckReport] = useAtom(epubCheckReportContext)
    ...
    // Checks whether the status is 'processing_finished' and if it is
    // then sets it to warning or success based on the epubCheckReport
    // If it's success, redirects to the editor in 2s
    useEffect(() => {
        if (status === 'processing_finished') {
            setStatus(epubCheckReport ? 'warning' : 'success')
        } else if (status === 'success') {
            setTimeout(() => {
                navigate(`/ebook/${fileId}`)
            }, 2000)
        }
    }, [epubCheckReport, fileId, navigate, status])
    ...
    return (
        <form
            ...
            <div className={uploading || fileId ? styles.hidden : ''}>
            ...
            <div
                className={
                    epubCheckReport && status === 'warning'
                        ? styles.warning_box
                        : styles.hidden
                }>
                <h3>Warning!</h3>
                <p>
                    The EPUB you just uploaded failed epubcheck. Our tool might
                    not work perfectly with it. You can download the error
                    report below.
                </p>
                <a
                    href={
                        'data:attachment/text,' +
                        encodeURI(epubCheckReport.replaceAll(',', ',\n'))
                    }
                    target="_blank"
                    download="epubcheck_report.txt"
                    rel="noreferrer">
                    Download epubcheck report
                </a>
                <button
                    type="button"
                    onClick={() => {
