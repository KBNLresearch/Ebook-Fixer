...
import { highlightText, ImageInfo } from '../../helpers/EditorHelper'
import { ReactComponent as CopySVG } from '../../assets/svgs/copy.svg'
import { renditionContext } from '../../helpers/EbookContext'
...
function AIAnnotator({
    ...
    const [rendition] = useAtom(renditionContext)
        ...
        if (aiAnnotationList.length > 0) {
            ...
            aiAnnotationList.forEach((el) => {
                if (el.type === 'CXT_YAKE_LAB') {
                    ...
                    // can be 'BB_GOOGLE_LAB' or 'BB_AZURE_LAB'
    ...
    return (
        <div className={styles.ai_control}>
            {aiChoice && aiChoice !== 'skipped' && aiChoice !== 'Invalid' ? (
                <div>
                    <label htmlFor="AiLabelsBox" className={styles.box_label}>
                        {' '}
                        Generated labels{' '}
                    </label>
                    <div className={styles.ai_labels_box} id="AiLabelsBox">
                        {imageAiLabs.map((obj, i) => (
                            <p className={getProportionalClass(obj)}>
                                {obj.text +
                                    (i === imageAiLabs.length - 1 ? '' : ',')}
                            </p>
                        ))}
                    </div>
                </div>
            ...
            {aiChoice === 'BB_AZURE_LAB' ? (
                <div>
                    <label htmlFor="AiSentenceBox" className={styles.box_label}>
                        {' '}
                        <br /> Generated description{' '}
                    </label>
            ...
            <label htmlFor="CxtKeywordsBox" className={styles.box_label}>
                {' '}
                <br /> Textual context keywords{' '}
            </label>
            <div className={styles.ai_labels_box} id="CxtKeywordsBox">
                {cxtKeywords.length > 0
                    ? cxtKeywords.map((obj, i) => (
                          <button
                              type="button"
                              onClick={() => {
